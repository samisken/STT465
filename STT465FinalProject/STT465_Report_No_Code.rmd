...
```{r Read in gavote data, echo=FALSE}
data("gavote")
...
```{r Surface Level EDA, echo=FALSE}
...
```{r Load GA County Level Data, echo=FALSE}
...
```{r Georgia Election Undercount Visualization, echo=FALSE}
# Visualizations
...
```{r Distribution of Dependant,echo=FALSE}
...
```{r Distribution of Categorical Variables,echo=FALSE}
...
```{r Stepwise Variable Selection, echo=FALSE}
...
```{r Stepwise Variable Selection Model 2, echo=FALSE}
...
```{r Removing High Leverage Outlier, echo=FALSE}
# Remove Fulton
remove <- c("FULTON")
...
```{r Regression Model Run,  echo=FALSE}
stepwise_lm_2 <- lm(undercount~equip+econ+gore+bush+ballots,data=gavote)
summary(stepwise_lm_2)
par(mfrow=c(2,2))
plot(stepwise_lm_2)
