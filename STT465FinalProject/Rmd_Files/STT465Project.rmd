...
The RMarkDown below contains the code as well as parts of the essay portions of this project (ones that are relevant to the coding scheme). I have also included a supplemental dataset I used to examine the election results at the county level of granularity. The other PDF contains the written report.

First let us load our necessary libraries.  
...
Then let us load our necessary data set (as referenced by STT 465 Project Rubric).
...
# STT 465: Final Project Rubric and Guidelines (Not included in Final Paper)
...
Please read the following instructions carefully and make sure you use the correct data set to do your own project. If you use a wrong data set for the project, you will automatically lose 40 points.
...
# Data Documentation - Final Project Project # 1 (STT 465 Documentation) (Not included in Final Paper)
...
in R to learn more about the variable names and dataset.
...
2. Obtain the relative undercount: (in percent of ballots in county)

```{r Create Undercount Variable}
...
Let us now conduct some surface level analysis of the data and the Georgia election as a whole.
...
As we can see the row names are specific counties in Georgia. This is useful if we have to remove any rows. 

Now let us do some data cleaning (check for NA values etc.)
```{r}
# Data Cleaning
...
In order to make inference and run necessary analysis (confidence intervals etc.) prior to our Bayesian analysis we will want our dependent variable to be normally distributed. This is close enough (based on the histogram above). Our dependent variable is roughly normal with a few right side outliers.
...
We can see that undercount is correlated with the class 'poor' in econ but, surprisingly, not clearly correlated with any single type of election equipment.

Now let us create specific prediction probabilities to  model likely scenarios. This code chunk is shown so the reader can clearly see how we developed these predictions.   
...
Now, for rigor, let us implement this using a matrix. 

```{r Matrix OLS Regression}
##########################################################
##########################################################
################## OLS Using Matrix  #####################
...
The above computation is included purely for rigor in order to show that, within R, there are multiple ways to implement a linear regression. It is not included in the final paper as it mimics the lm() function. 
...
Let's now convert B and varE to mcmc objects in order to examine autocorrelation using the coda library. This will allow us to run more analysis. 
...
In both instances the autocorrelation approaches 0 as Lag increases to 20 Thus we should sample 1 value every 20 indexes to control for autocorrelation. Thus, we should resample with value 20,40 , .. to n hundred (end of our data set). This process is called thinning our data.
